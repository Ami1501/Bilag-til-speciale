...
############ RÅDATA ############
#Industridata
Raw_daily <- read.csv("C:/Users/ander/OneDrive - CBS - Copenhagen Business School/Speciale/Data/Data/49_Industry_Portfolios_Daily_EW.csv"); names(Raw_daily)[1] <- "Date"
Raw_Monthly <- read.csv("C:/Users/ander/OneDrive - CBS - Copenhagen Business School/Speciale/Data/Data/49_Industry_Portfolios_Monthly_EW.csv"); names(Raw_Monthly)[1] <- "Date"
...
l<-lm(a_sim[3:51]*250/100~beta_sim[3:51]);   abline(l$coefficients[1],l$coefficients[2],col="Blue")



############ Kompliceret beregning med konstant w=0.91 ############ 
...
for (j in (1+Forskydning):(c+Forskydning)) {
  ...
  repeat {
    ...
    if ( anyNA(c(Std, Cor))) {
      ...
      shrinkBeta <- 0.907*beta + (1-0.907)*1 #Justeret beta mod 1   0.907 er fra  det totale gennemsnit af wit senere
...
print(xtable(sum3fak, type = "latex"), file = "LigeVærdi w09 b1 3fak.tex") #Gemmes 
...
############ Tabel Oversigt for P1-P10 og BAB ############ 
...
plot(ts(rowMeans(wit[,],na.rm = TRUE), start=c(1934,4), end=2021, frequency = 12), type='l',col="black",xlab="Tid",ylab="wt",cex.lab=0.3,las=1)
abline(h=w)


par(mar=c(2, 4, 1, 5))
plot(ts(betaTilplot[,1], start=c(1929,3), end=2021, frequency = 12), ylim=c(0.3,2.7), xlab="Tid", ylab="Beta", type="l",col="red",las=1)
par(new = TRUE)
plot(ts(betaTilplot[,2], start=c(1929,3), end=2021, frequency = 12), axes=FALSE, ylim=c(0.3,2.7), xlab="", ylab="", type="l",col="green",las=1)
#axis(2, ylim=c(0.3,2.5),col="black",las=1)  ## las=1 makes horizontal labels
par(new = TRUE)
plot(ts(rowMeans(wit[,],na.rm = TRUE), start=c(1934,4), end=2021, frequency = 12), axes=FALSE, ylim=c(0.70,1.00), type='l',col="black",xlab="",ylab="")
mtext("wt",side=4,col="black",line=3)
axis(4,c(0.7,0.8,0.9,1),col="black",las=1,line=0)  ## las=1 makes horizontal labels
legend("topright",c("Høj-beta", "Lav-Beta","wt"), lwd=c(1,1,1), col=c("red","green","black"),box.lwd = 0,bg="transparent",ncol = 3,box.col="transparent")




#Test for uafhængighed af data 
Box.test(rowMeans(wit[,],na.rm = TRUE),lag=1,type="Ljung-Box",fitdf=1) #Afviser => autocorrelation

#Test for stationaritet
kpss.test(rowMeans(wit[,],na.rm = TRUE),null="Level") #accepterer => stationær
adf.test(rowMeans(wit[,],na.rm = TRUE)) #afviser unitroot => stationær
...
TEDTemp <- readxl::read_excel("C:/Users/ander/OneDrive - CBS - Copenhagen Business School/Speciale/Data/Data/TEDRATE.xls")
...
# Bergn månedlig data for k
# Min 750 dage for corr data
currentIndex <- 1
k <- 1


startIndex <- currentIndex
startTED <- 0.9 #  Den første observation, som er hardcoded
currentMonth <- substring(rownames(TED)[startIndex],1,7)
colnames(TEDstat)[1] <- 'Volatility i mmåneden'
colnames(TEDstat)[2] <- 'TED spread i slutningen af sidste måned'
colnames(TEDstat)[3] <- 'Ændring i TED spread i løbet af måneden'
repeat {
  # Find sidste måned
  ...
  endTED <- TED[endIndex,1] # Sidste værdi i måned t-1
  # Volatilitet for måneden:
  row.names(TEDstat)[k] <- currentMonth
  TEDstat[k,1] <- sd(TED[startIndex:endIndex,1])
  TEDstat[k,2] <- startTED 
  TEDstat[k,3] <- endTED - startTED 
  
  # Opdatere værdier, så næste måneds volatilitet kan regnes
  startIndex <- currentIndex # Første dag i måned t
  startTED <- TED[startIndex - 1,1] # Sidste værdi i måned t-1
  currentMonth <- substring(rownames(TED)[startIndex],1,7) # måned t
  
  # skippe sidte observation
...
laggedTED <- TEDstat[(1+LaggedForskydning):420,2] * 100 # bps
changeInTED <- TEDstat[(1+LaggedForskydning):420,3] * 100 # bps
betaSpread <- BABpf[(nTedStart-1+LaggedForskydning):nTedEnd,7] * 100 # ikke bp, bare skalerert
...
# Plot af TED
par(mar=c(2, 4, 1, 1)) #uden xlab
plot(ts(TEDstat[,2]*100,start= 1986, end = 2021, frequency = 12),type = 'l', xlab= "Tid", ylab="TED spread i bps",las=1)
...
Pvol<- data.frame(matrix(data=0, nrow=10, ncol=1))
...
for (j in 1:10){
  ...
  Pvol[j,1] <- (sd(Ppre[,10+j])*sqrt(12)) #Månedlig
...
for (j in 1:10){
  ...
  Pvol[j,1] <- (sd(Ppost[,10+j])*sqrt(12)) #Månedlig
  }
...
xx <- barplot(t(PorteføljerMerPrePost), beside=T, #main="Jensens alfa fordelt ud på P1 - P10",
              ylab="Merafkast",xlab="Porteføljer",col=c("darkgreen","darkblue"),names.arg=c("P1", "P2", "P3", "P4", "P5", "P6", "P7", "P8", "P9", "P10"),ylim=c(-0.20,1.6))
#text(x = xx, y = PogBABAlpha, label = round(PogBABAlpha,2), pos = c(3,3,3,3,3,3,3,3,3,1,3), cex = 0.8, col = c("darkgreen","darkgreen","darkgreen","darkgreen","darkgreen","darkgreen","darkgreen","darkgreen","darkgreen","red","darkgreen"))
#legend(37,0.45, c("Alfa","3-faktor alfa"),pch=c(15,15),col=c("darkgreen","limegreen"),pt.cex=2,box.col="white")
legend("topleft", c("Præ CAPM","Post CAPM"),pch=c(15,15),col=c("darkgreen","darkblue"),pt.cex=2,box.col="transparent",box.lwd = 0,bg="transparent",ncol = 1)
box(lty = 'solid', col = 'black')
...
par(mar=c(2, 4, 1, 1)) #uden xlab
plot(ts(BABpf[,4],start=c(1934,2), end=2021, frequency = 12),ylab="Kumuleret BAB-afkast i $",xlab="Tid",las=1) #Tror den hopper i starten grundet negativt cummulative afkast, og log til negativt er lort :)
...
#Pre CAPM
Raw_daily_mer[10799,1]
row.names(betasNon)[430]
...
apost1 <- colMeans(Raw_daily_mer[10800:n_Daily,(1+Forskydning):(c+Forskydning)],na.rm=TRUE)/100*250
...
Raw_daily_mer[18097,1]
row.names(betasNon)[778]
#Pre FF
apre1 <- colMeans(Raw_daily_mer[2272:18097,(1+Forskydning):(c+Forskydning)],na.rm=TRUE)/100*250
bpre1 <- colMeans(betasNon[60:778,1:c],na.rm = TRUE)

apost1 <- colMeans(Raw_daily_mer[18098:n_Daily,(1+Forskydning):(c+Forskydning)],na.rm=TRUE)/100*250
bpost1 <- colMeans(betasNon[779:AntalMåneder,1:c],na.rm = TRUE)
...
abline(0,apost1[1],col="darkblue",lty=2)
l<-lm(apost[1:49]~bpost[1:49]);   abline(l$coefficients[1],l$coefficients[2],col="darkblue",lwd=1)
legend(0,0.3,c("Præ smart beta empirisk SML","Post smart beta  empirisk SML","Præ smart beta  teoretisk SML","Post smart beta  teoretisk SML"),col=c("darkgreen","darkblue","darkgreen","darkblue"),lwd=c(2,2,2,2),lty=c(1,1,2,2),box.col = "transparent",bg="transparent",ncol=1,cex = 0.8)





Raw_daily_mer[23133,1]
row.names(betasNon)[1018]
#Pre BAB
apre1 <- colMeans(Raw_daily_mer[2272:23133,(1+Forskydning):(c+Forskydning)],na.rm=TRUE)/100*250
bpre1 <- colMeans(betasNon[60:1018,1:c],na.rm = TRUE)

apost1 <- colMeans(Raw_daily_mer[23134:n_Daily,(1+Forskydning):(c+Forskydning)],na.rm=TRUE)/100*250
bpost1 <- colMeans(betasNon[1019:AntalMåneder,1:c],na.rm = TRUE)
...
abline(0,apost1[1],col="darkblue",lty=2)
l<-lm(apost[1:49]~bpost[1:49]);   abline(l$coefficients[1],l$coefficients[2],col="darkblue",lwd=1)
legend(0,0.3,c("Præ BAB empirisk SML","Post BAB  empirisk SML","Præ BAB  teoretisk SML","Post BAB  teoretisk SML"),col=c("darkgreen","darkblue","darkgreen","darkblue"),lwd=c(2,2,2,2),lty=c(1,1,2,2),box.col = "transparent",bg="transparent",ncol=1,cex = 0.8)
...
AQR_sorteret <- as.data.frame(read_excel("C:/Users/ander/OneDrive - CBS - Copenhagen Business School/Speciale/Data/Data/AQR sorteret.xlsx", 
...
############# Præ og post Smart beta (kumuleret ,erafkast) ############# 

AQR_sorteret$kum =0
for(i in (40:nrow(AQR_sorteret))){
  AQR_sorteret$kum[i] <- AQR_sorteret$kum[i-1]+ AQR_sorteret$Return[i]
}
...
AQR_sorteret$kum2 =0
for(i in (758:nrow(AQR_sorteret))){
  AQR_sorteret$kum2[i] <- AQR_sorteret$kum2[i-1]+ AQR_sorteret$Return[i]
}

BABSM<- data.frame(matrix(data=0,nrow=AntalMåneder-718-60,ncol=1))
BABSM[1,1]=BABpf[720,3]

for(i in (2:(AntalMåneder-718-60))){
  row.names(BABSM)[i-1] <- row.names(BABpf)[i+718]
 BABSM[i,1] <- BABpf[i+719,3]+ BABSM[i-1,1]
}
...
par(mar=c(2, 4, 1, 0)) 
plot(ts(AQR_sorteret$kum[39:757],start=c(1934,2), end=c(1993,12), frequency = 12),ylim=c(-20,470),las=1,ylab="Kumuleret afkast i $")
par(new=TRUE)
plot(ts(BABpf[1:719,4],start=c(1934,2), end=c(1993,12), frequency = 12),ylim=c(-20,470),col="red",las=1,ylab="Kumuleret afkast i $")
legend("topleft",c("Præ SB AQR","Præ SB (vores)"),col=c("black","red"),lwd=c(2,2),lty=c(1,1),box.col = "transparent",bg="transparent",ncol=1,cex = 0.8)


par(mar=c(2, 2, 1, 1)) 
plot(ts(AQR_sorteret$kum2[758:1081],start=c(1994,1), end=c(2020,12), frequency = 12),ylim=c(-20,470),las=1,ylab=NULL,yaxt = "n")
par(new=TRUE)
plot(ts(BABSM[,1],start=c(1994,1), end=c(2020,12), frequency = 12),ylim=c(-20,470),col="red",las=1,ylab=NULL,yaxt = "n")
legend("topleft",c("Post SB AQR","Post SB (vores)"),col=c("black","red"),lwd=c(2,2),lty=c(1,1),box.col = "transparent",bg="transparent",ncol=1,cex = 0.8)




par(mfrow=c(1,1))
plot(ts(AQR_sorteret$kum[40:1081],start=c(1934,4), end=c(2020,12), frequency = 12),ylim=c(0,800))
