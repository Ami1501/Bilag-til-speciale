...
#Tilføjer returns
Raw_daily$MarketReturn <- FF3d[,7]  #Ændre daily til 7 hvis vi bruger ligevægtet marketsafkast
Raw_Monthly$MarketReturn <- FF3m[,7]  #Ændre daily til 7 hvis vi bruger ligevægtet marketsafkast
...
for (j in (1+Forskydning):(c+Forskydning)) {
  ...
  repeat {
    ...
    if ( anyNA(c(Std, Cor))) {
      ...
      shrinkBeta <- 0.916*beta + (1-0.916)*1.07993 #Justeret beta mod 1    0.74 er fra wi, 1.08 er fra meanbeta
...
xx <- barplot(JAlpha[,1], #main="Jensens alfa fordelt ud på P1 - P10",
              ylab="CAPM alfa",xlab="Porteføljer",col=c("darkblue"),names.arg=c("P1", "P2", "P3", "P4", "P5", "P6", "P7", "P8", "P9", "P10"),ylim=c(-0.45,0.5))
text(x = xx, y = JAlpha[,1], label = round(JAlpha[,1],2), pos = c(3,3,3,3,3,1,3,1,1,1), cex = 0.8, col = c("darkgreen","darkgreen","darkgreen","darkgreen","darkgreen","red","darkgreen","red","red","red"))
...
legend(1927,3000000,c("Lav-beta", "Middel-Beta","Høj-Beta","Markedet"), lwd=c(1,1), col=c("green","red","blue","black"))
...
legend(1927,2500000,c("Lav-beta", "Middel-Beta","Høj-Beta","Markedet","BAB"), lwd=c(1,1), col=c("green","red","blue","black","gold"))
...
print(xtable(tabel, type = "latex"), file = "LigeLige tabel.tex") #Gemmes som "filename2"
